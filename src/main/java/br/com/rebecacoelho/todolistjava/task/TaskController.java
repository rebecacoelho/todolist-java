...
import br.com.rebecacoelho.todolistjava.utils.Utils;
...
public class TaskController {
    ...
    // http://localhost:8080/tasks/892347823-cdfgvb-832748234 - id do usuário
    @PutMapping("/{id}")
    public ResponseEntity update(@RequestBody TaskModel taskModel, @PathVariable UUID id, HttpServletRequest request) {
        ...
        if (!task.getIdUser().equals(idUser)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Usuário não tempermissão para alterar essa tarefa"); 
        }
